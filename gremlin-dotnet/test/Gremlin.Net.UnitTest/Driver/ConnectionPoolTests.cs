...
namespace Gremlin.Net.UnitTest.Driver
{
    public class ConnectionPoolTests
    {
        ...
        [InlineData(1)]
        [InlineData(2)]
        public void ShouldPerformConfiguredNrRetriesForUnavailableServer(int nrRetries)
        {
            var mockedConnectionFactory = new Mock<IConnectionFactory>();
            mockedConnectionFactory.Setup(m => m.CreateConnection()).Returns(ClosedConnection);
            var pool = CreateConnectionPool(mockedConnectionFactory.Object, 1, nrRetries);

            Assert.ThrowsAny<Exception>(() => pool.GetAvailableConnection());

            mockedConnectionFactory.Verify(m => m.CreateConnection(), Times.Exactly(nrRetries + 2));
        ...
        private static ConnectionPool CreateConnectionPool(IConnectionFactory connectionFactory, int poolSize = 2,
            int getOpenConnectionRetries = 1)
        {
            return new ConnectionPool(connectionFactory,
                new ConnectionPoolSettings {PoolSize = poolSize, GetOpenConnectionRetries = getOpenConnectionRetries});
        }
    }
}